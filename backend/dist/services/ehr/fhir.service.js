...
exports.createFHIRService = createFHIRService;
...
class FHIRService {
    constructor(accessToken) {
        this.baseUrl = env_1.env.epicFhirBaseUrl;
        this.client = axios_1.default.create({
            ...
            headers: {
                ...
                'Epic-Client-ID': env_1.env.auth0ClientId,
            },
        });
    }
    /**
    ...
    async getPatient(patientId) {
        logger_1.logger.info(`[FHIR] Fetching patient: ${patientId}`);
        ...
        const fullName = [
            ...
            .filter(Boolean)
            .join(' ');
        const memberId = p.identifier?.find((id) => id.system?.includes('member') || id.system?.includes('insurance'))?.value || `MBR-${patientId.slice(0, 8).toUpperCase()}`;
        return {
            patientId,
            name: fullName,
    ...
    async getMedicationRequests(patientId) {
        logger_1.logger.info(`[FHIR] Fetching medication requests for patient: ${patientId}`);
        const response = await this.client.get(`/MedicationRequest?patient=${patientId}&status=active&_count=50`);
    ...
    async getConditions(patientId) {
        logger_1.logger.info(`[FHIR] Fetching conditions for patient: ${patientId}`);
        const response = await this.client.get(`/Condition?patient=${patientId}&clinical-status=active&_count=50`);
        return (response.data.entry
            ?.map((e) => e.resource)
...
function createFHIRService(accessToken) {
