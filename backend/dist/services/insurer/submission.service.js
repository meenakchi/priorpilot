...
class SubmissionService {
    async submitPriorAuth(insurerId, form) {
        ...
        await sleep(1500);
        const referenceNumber = `PA-${Date.now()}-${(0, uuid_1.v4)().slice(0, 6).toUpperCase()}`;
        ...
        logger_1.logger.info(`[Submission] PA submitted. Reference: ${referenceNumber}`);
        return result;
    }
    validateForm(form) {
        const required = [
...
function getBusinessDaysFromNow(insurerId) {
    const daysMap = {
        BCBS: 5,
        AETNA: 3,
        UNITEDHEALTHCARE: 2,
